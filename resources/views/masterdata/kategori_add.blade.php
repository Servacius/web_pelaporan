          ...
          <form method="post" action="{{ route('kategori.store') }}" autocomplete="off" class="form-horizontal">
            ...
            <div class="card ">
              <div class="card-header card-header-primary">
                <h4 class="card-title">{{ __('Add Kategori') }}</h4>
                <p class="card-category">{{ __('Tambah Kategori Barang') }}</p>
              </div>
              <div class="card-body ">
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="input-kategori" type="text" placeholder="{{ __('Nama Kategori') }}" required="true" aria-required="true"/>
                      @if ($errors->has('name'))
